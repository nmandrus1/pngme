...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> anyhow::Result<Chunk> {
        for (i, chunk) in self.chunks.iter().enumerate() {
            if chunk.chunk_type().eq(&ChunkType::from_str(chunk_type)?) { return Ok(self.chunks.remove(i))
            }
        }
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<Chunk> {
        let ctype = match ChunkType::from_str(chunk_type) {
            Ok(c) => c,
            Err(e) => return None,
        };

        for (i, chunk) in self.chunks.iter().enumerate() {
            if chunk.chunk_type().eq(&ctype) {
                return Some(self.chunks[i].clone())
            }
        }
    ...
    pub fn chunks_by_type(&self, chunk_type: &str) -> Option<impl Iterator<Item = &Chunk>> {
        let ctype = match ChunkType::from_str(chunk_type) { Ok(c) => c,
            Err(e) => panic!("{}", e),
        };
        let iter = self.chunks.iter().filter_map(move |c| {
            if c.chunk_type() == &ctype { 
                Some(c)
            } else { None }
        });
        if iter.clone().count() == 0 { return None }
        Some(iter)
    }

    pub fn as_bytes(&self) -> Vec<u8> {
        Self::STANDARD_HEADER.iter().copied()
            .chain(self.chunks.iter().flat_map(|c| c.as_bytes())).collect()
    }
}
...
impl std::convert::TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> anyhow::Result<Self, Self::Error> {
        if !bytes[0..8].iter().enumerate().all(|(i, b)| b == &Png::STANDARD_HEADER[i]) {
            return Err(anyhow::anyhow!("Invalid PNG Header"))
        }

        let bytes_len = bytes.len();
        let mut idx = 8;
        let mut reader = std::io::BufReader::new(&bytes[8..12]);
        let mut buffer: [u8; 4] = [0, 0, 0, 0];
        reader.read_exact(&mut buffer)?;
        let mut length = u32::from_be_bytes(buffer) + 12; //add 12 for the 12 other bytes in a Chunk
        let mut chunks: Vec<Chunk> = Vec::with_capacity(50);

        while idx + (length as usize) < bytes_len {
            chunks.push(Chunk::try_from(&bytes[idx..idx + length as usize])?);
            idx = idx + length as usize;
            reader = std::io::BufReader::new(&bytes[idx..idx + 4]);
            reader.read_exact(&mut buffer)?;
            length = u32::from_be_bytes(buffer) + 12;
        }
         
        // Add last chunk in the file
        chunks.push(Chunk::try_from(&bytes[idx..idx + length as usize])?);
...
impl std::fmt::Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        self.as_bytes().chunks(10).for_each(|c| {
            c.iter().for_each(|b| write!(f, "{:#6X}", b).unwrap());
            writeln!(f).unwrap();
        });
        Ok(())
    }
